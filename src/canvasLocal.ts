export class CanvasLocal {
  ...
  drawface(x1:number,y1:number,x2:number,y2:number,x3:number,y3:number,x4:number,
    y4:number,x5:number,y5:number,x6:number,y6:number,x7:number,y7:number,x8:number,y8:number,
    color:string):void{
    this.graphics.fillStyle = color;
    this.graphics.beginPath();
    this.graphics.moveTo(x1, y1);
  ...
  paint():void {
    ...
    //for (let i = 1.3; i <= 8; ) {
      ...
      ind++;
      this.drawHexface(this.iX(5.3), this.iY(6 * h[ind] / maxEsc), this.iX(1) - this.iX(0), this.iY(6 * h[ind] / maxEsc) - this.iY(0), 127, 255, 0);
      ind++;
